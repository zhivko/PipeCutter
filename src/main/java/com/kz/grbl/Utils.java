...
public class Utils {
	...
	public ArrayList<Integer> calculateCutPoints(MyPickablePoint clickedPoint,
		...
		MyPickablePoint tempPoint = findConnectedPoint(clickedPoint, alAlreadyAddedPoints);
	...
	public static ArrayList<Coord3d> CalculateLineLineIntersection(Coord3d line1Point1,
			Coord3d line1Point2, Coord3d line2Point1, Coord3d line2Point2) {
	...
	public String coordinateToGcode(MyPickablePoint point) {
		...
		if (moveRelatively) {
			...
			z = String.valueOf(point.xyz.z + Cylinder.offsetZ + SurfaceDemo.instance.plasmaStandoff)
					.replace(",", ".");
		} else {
	...
	public String coordinateToGcode(Coord3d coord) {
		boolean moveRelatively = false;
		String x, y, z;
		if (moveRelatively) {
			x = String.valueOf(Cylinder.offsetX - coord.x).replace(",", ".");
			y = String.valueOf(coord.y + Cylinder.offsetY).replace(",", ".");
			z = String.valueOf(coord.z + Cylinder.offsetZ + SurfaceDemo.instance.plasmaStandoff).replace(
					",", ".");
		} else {
	...
	public Coord3d rotate(double x, double y, double z, double angle, double axisX, double axisY,
			double axisZ) {
	...
	public void rotatePoints(double angle, boolean slow) {
		if (!slow) {
			...
			for (int i = 0; i <= noSteps; i++) {
				double value = Double.valueOf(SurfaceDemo.instance.angleTxt) + angle * i / noSteps;
				double[] zAxisDouble = { 0.0d, 1.0d, 0.0d };
				Vector3D zAxis = new Vector3D(zAxisDouble);
				Rotation rotZ = new Rotation(zAxis, Math.toRadians(value));
				for (MyPickablePoint point : SurfaceDemo.instance.utils.origPoints.values()) {
					...
					Vector3D result = rotZ.applyTo(myPoint);
					points.get(point.id).xyz.set(Double.valueOf(result.getX()).floatValue(),
							Double.valueOf(result.getY()).floatValue(), Double.valueOf(result.getZ())
									.floatValue());
				}
				for (MyEdge edge : continuousEdges.values()) {
					edge.calculateCenter();
	...
	public org.jzy3d.plot3d.primitives.Point calculateOffsetPoint(MyPickablePoint point) {
		...
		if (continuousEdge.edgeType == MyContinuousEdge.EdgeType.ONPIPE) {
			...
			try {
				...
				double vectorCrossLength = plane.getNormal().crossProduct(new Vector3D(0f, 1.0f, 0.0f))
						.getNorm();
				...
				Vector3D vecPrevPrevPoint = new Vector3D(prevPrevPoint.xyz.x, prevPrevPoint.xyz.y,
						prevPrevPoint.xyz.z);
