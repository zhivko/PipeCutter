...
handle_command_input(zloop_t *loop, zmq_pollitem_t *poller, void *arg)
{
	...
	if(f!=NULL)
	{
	char *frameStr = zframe_strhex(f);
	std::string myString;
	size_t size;
	//myString = string(frameStr);
	size = zframe_size(f);
	int iSize = static_cast<int>(size);
	fprintf(stderr,"zframe_size: %d content: %s\n", iSize, frameStr);

	//if (!self->rx.ParseFromArray(zframe_data(f), zframe_size(f))) {
	if (!self->rx.ParseFromArray(zframe_data(f), size)) {
		   ...
		   const machinetalk::Component *pbcomp, void *socket)
{
    ...
    if (!pbcomp->has_name()) {
		fprintf(stderr,"Klemen zframe_size process_rcomp_bind 2");
	...
	if (!(p.has_name() &&
	    ...
	    gpb::TextFormat::PrintToString(p, &s);
        zframe_t *o = zmsg_first (from);  // freed with msg
		fprintf(stderr,"Klemen zframe_size process_rcomp_bind 3");
        std::string origin( (const char *) zframe_data(o), zframe_size(o));
	    note_printf(self->tx,
    ...
    if (pbcomp->has_no_create() && pbcomp->no_create()) {
        zframe_t *o = zmsg_first (from);  // freed with msg
		fprintf(stderr,"Klemen zframe_size process_rcomp_bind 4");
        std::string origin( (const char *) zframe_data(o), zframe_size(o));
        note_printf(self->tx,
...
dispatch_request(htself_t *self, zmsg_t *from, void *socket)
{
	...
	if (self->rx.comp_size() == 0) {
        zframe_t *o = zmsg_first (from);  // freed with msg
		fprintf(stderr,"Klemen zframe_size process_rcomp_bind 5");
    ...
    zframe_t *o = zmsg_first (from);  // freed with msg
	fprintf(stderr,"Klemen zframe_size process_rcomp_bind 6");
